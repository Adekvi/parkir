@foreach ($terima as $index => $kolektor)
    <div class="modal fade text-left" id="setuju{{ $kolektor['id_kolektor'] }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel160" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    <h5 class="modal-title white" id="myModalLabel160">Approve Kolektor</h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ url('kasir/dataSetoran-status/' . $kolektor['id_kolektor']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
